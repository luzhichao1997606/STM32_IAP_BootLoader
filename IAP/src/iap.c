...
uint8_t APP_DATA_EXIST_Flag = 0;
/************************************************************************/
//如果检测到不是只有BL，则可以通过是否有APP程序进行判断，是运行APP还是加入菜单界面。
void IAP_Check_Top_Adder()
{
	...
	if (((*(__IO uint32_t*)ApplicationAddress) & 0x2FFE0000 ) == 0x20000000){
				...
				APP_DATA_EXIST_Flag = 1;
		
				strcpy((char *)cmdStr,CMD_RUNAPP_STR);
		} 
}
